...
class TestCfgReclassification(unittest.TestCase):
    ...
    @staticmethod
    def _assert_models_equal(model_a: "CFGModel", model_b: "CFGModel"):
        assert model_a.graph.nodes() == model_b.graph.nodes()
        assert model_a.graph.edges() == model_b.graph.edges()
        # FIXME: Check more

    @staticmethod
    def _assert_function_graphs_equal(function_a: "Function", function_b: "Function"):
    ...
    def _assert_all_function_equal(self, functions_a: "FunctionManager", functions_b: "FunctionManager"):
        ...
        for f in functions_a:
            ...
            self._assert_function_graphs_equal(functions_a[f], functions_b[f])
            # FIXME: Check more (eg edges)
    ...
    def test_cfgfast_combine_with_full_model(self):
        ...
        self._assert_models_equal(cfg_a.model, cfg_b.model)
        self._assert_all_function_equal(cfg_a.functions, cfg_b.functions)
...
class TestCfgPatching(unittest.TestCase):
    ...
    def _assert_cfgs_equal(expected, result):
        ...
        for n in nodes_result:
            if n not in nodes_expected:
                ...
                if nodes_result[n] != nodes_expected[n]:
                    log.error(
                        "Test result graph node at %#x has size %#x bytes, expected %#x bytes",
                        n,
                        nodes_result[n],
                        nodes_expected[n],
                    )
        ...
        assert expected.model.graph.edges() == result.model.graph.edges()
    ...
    def _test_patch(self, patches: Sequence[Tuple[int, str]]):
        ...
        with tempfile.NamedTemporaryFile(prefix="fauxware-patched-", delete=False) as f:
            ...
            cfg_after_patching = proj.analyses[CFGFast].prep()(
                **common_cfg_options, model=cfg_before_patching.model, base_state=proj.kb.patches.patched_blank_state
            )

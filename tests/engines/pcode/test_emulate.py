...
# XXX: Probably move to make real pypcode classes instantiable
@dataclass(eq=True)
class MockAddrSpace:
    name: str
...
class TestPcodeEmulatorMixin(unittest.TestCase):
    def _step_irsb(self, irsb, state=None):
    ...
    #                     OpCode.LOAD,
    #                     VN(UNIQUE_SPACE, 0, 8),
    #                     [VN(CONST_SPACE, 0xCACACACA, 0, space_encoded_in_offset=RAM_SPACE), VN(RAM_SPACE, 0, 8)],
    #                 ),
    #             ],
    #         )
    #     )
    ...
    def test_bool_negate(self, input_value, expected_value):
